...
/**
 * ldc / invokedynamic 字符串常量改写器（ASM ClassVisitor）。
 *
 * <p>职责：把类字节码中命中类翻译表的 String 型 ldc 字面量与 indy bootstrap
 * 字符串参数（Java 9+ 字符串拼接 recipe）替换为译文。只换 String→String 常量，
 * 栈型与帧不变，故 {@code ClassWriter} 不需重算帧；构造时传入原
